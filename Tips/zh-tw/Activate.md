...
*如果您在啟用時失敗，請先確認您的操作是否有誤。若您十分確定您的操作無誤，建議您檢查您的系統是否有曾經使用盜版 KMS 啟用器（等），若確實如此，請重灌您的系統。*
...
這個功能您必須擁有電話（手機），請在《金鑰管理》中（點開摺疊），輸入您在電話中所聽到的數字，並點選「安裝確認識別碼」（在「安裝金鑰」按鈕旁的「更多功能」（下三角形）），右側將會顯示啟用的過程及啟用結果。**請注意，產品、授權必須與您輸入的相對應。**
